...
let checkConstraint (constraints : Map<coord, (char -> bool)>) (coord : coord) (c : char) : bool = failwith "Not implemented"

let getValidNeighbours (board : board) (pp : Map<coord, char>) (c : coord) : coord list = 
    let neighbours : coord list = [(fst c, snd c + 1); (fst c, snd c - 1); (fst c + 1, snd c); (fst c - 1, snd c); (0, 0)]
...
let generateHLines (pp : Map<coord, char>) (board : board) (coords : coord list) : (coord * coord) list = 
    let f (c : coord) : (coord * coord) =
        ...
        (stretchRight c 3, stretchLeft c 0)
...
let generateHConstraints (dict : Dictionary) (pp : Map<coord, char>) (board : board) (coords : coord list) : Map<coord, (char -> bool)> = failwith "Not implemented"

let generateVConstraints (dict : Dictionary) (pp : Map<coord, char>) (board : board) (coords : coord list) : Map<coord, (char -> bool)> =
...
let generateHMove (dict : Dictionary) (pp : Map<coord, char>) (line : coord * coord) (constraints : (coord -> char -> bool)) (hand : MultiSet<(uint32 * char list)>) : move list = 
    
    let rec folder (c : coord) (move : move) (dict : Dictionary) (hand : MultiSet<(uint32 * char list)>) (moves : move list) (piece : (uint32 * char list)) (_ : uint32) : move list = 
    ...
    and multiFold (c : coord) (move : move) (dict : Dictionary) (hand : MultiSet<(uint32 * char list)>) = 
        MultiSet.fold (folder c move dict hand) List.empty hand 

    let moves = multiFold (fst line) List.empty dict hand
    moves

let generateVMove (dict : Dictionary) (pp : Map<coord, char>) (line : coord * coord) (constraints : (coord -> char -> bool)) (hand : MultiSet<(uint32 * char list)>) : move list = failwith "Not implemented"


let generateMoves (dict : Dictionary) (pp : Map<coord, char>) (board : board) (hand : MultiSet<(uint32 * char list)>) : move list = 
