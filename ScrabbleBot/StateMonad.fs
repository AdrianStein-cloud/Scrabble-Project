module StateMonad
    ...
    let pop : SM<unit> =
        S (fun s -> 
            match s.vars with
            | [] -> Failure (ReservedName "Cannot pop last variable environment")
            | _ :: vars -> Success ((), { s with vars = vars }))


    let wordLength : SM<int> =
        S (fun s -> 
            let len = List.length s.word 
            Success (len, s))
    ...
    let pointValue (pos : int) : SM<int> =
        S (fun state ->
            match List.tryItem pos state.word with
            | None -> Failure (IndexOutOfBounds pos)
            | Some (_, v) -> Success (v, state))
